...
int track_motor(void) { // move motor feedback pot/encoder to setpoint
    ...
    do { // move the assy/motor into the setting position
        ...
        if ((motordata[track_knob_to_pot].pot.error > (int) ((float) TRACK_DB_L / deadband)) && (motordata[track_knob_to_pot].pot.error < (int) ((float) TRACK_DB_H / deadband))) {
            if (PERFECT && mode.qei) {
                ...
                if (was_running) {
                    ...
                    if (TLOG) {
                        sprintf(bootstr2, " Error %3i, SLOW Flag %1i, SLOW Relay %i, Total Hunts %i ", motordata[track_knob_to_pot].pot.error, motordata[track_knob_to_pot].slow, (int) POWER_S, motordata[track_knob_to_pot].hunt_count);
                        puts2USART(bootstr2);
                        putrs2USART("\x1b[7m Motor Stopped. \x1b[0m\r\n");
                    }
                    was_running = FALSE;
        ...
        if (motordata[track_knob_to_pot].pot.error > (int) ((float) TRACK_DB_H / deadband)) { // check knob position
            ...
            if ((old_error != motordata[track_knob_to_pot].pot.error) && (ABSL(motordata[track_knob_to_pot].pot.error) > TRACK_DISPLAY)) {
                if (TLOG) {
                    sprintf(bootstr2, " Error %3i, SLOW Flag %1i, SLOW Relay %i, Hunts %i ", motordata[track_knob_to_pot].pot.error, motordata[track_knob_to_pot].slow, POWER_S, motordata[track_knob_to_pot].hunt_count);
                    puts2USART(bootstr2);
    ...
    for (z = 0; z < times; z++) {
        for (m = 0; m < MAX_MOTOR; m++) {
            ...
            if (motordata[knob_to_pot].active) {
                ...
                motordata[knob_to_pot].pot.error = motordata[knob_to_pot].pot.pos_set - motordata[knob_to_pot].pot.pos_actual;
                track_motor();
                motordata[knob_to_pot].pot.scaled_set = 500;
                motordata[knob_to_pot].pot.error = motordata[knob_to_pot].pot.pos_set - motordata[knob_to_pot].pot.pos_actual;
                track_motor();
                if (button.B0 || button.B1 || button.B2) {
                    ret = 1;
...
void emo_display(void) { // System has detected a condition that caused it to stop in a save comdition
...
void set_all_motors(void) {
    int knob_to_pot_save = knob_to_pot;
    ...
    for (z = 0; z < MAX_MOTOR; z++) { // Lets look at the results of the calibration.
        ...
        if (!motordata[z].pot.cal_failed) {
            ...
            if (motordata[z].active) {
                ...
                LCD_VC_puts(VC0, DS2, YES);
                sprintf(bootstr2, "IF Offset %i < %i     ", motordata[z].pot.offset, motordata[z].pot.limit_offset);
                LCD_VC_puts(VC0, DS3, YES);
                blink_led(TEST_LED, TRUE);
                do {
                    buzzer_ticks(2);
