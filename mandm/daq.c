...
void Reset_Change_Count(void) // counts for ADC value change
{
        change_count = 0;
        R.change_x = R.pos_x;
        R.change_y = R.pos_y;
...
BYTE Change_Count(void)
{
        if (change_count++ >= CHANGE_COUNT) {
                if (ABSL(R.pos_x - R.change_x) < MIN_CHANGE) R.stable_x = TRUE;
                if (ABSL(R.pos_y - R.change_y) < MIN_CHANGE) R.stable_y = TRUE;
                if (ABSL(R.pos_z - R.change_z) < MIN_CHANGE) R.stable_z = TRUE;
...
{ // ADC is opened and config'd in main
        static unsigned int i, z, change = 0; // used for fast and slow sample loops >256
